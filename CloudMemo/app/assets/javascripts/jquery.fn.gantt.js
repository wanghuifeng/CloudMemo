...
(function ($) {
    ...
    $.fn.gantt = function (options) {
        ...
        var scales = ["minutes", "hours", "days", "weeks", "months"];
        //Default settings
        var settings = {
            source: null,
            itemsPerPage: 7,
        ...
        var core = {
            ...
            init: function (element) {
                element.rowsNum = element.data.rowsNum || element.data.length;
                ...
                element.pageNum = element.data.pageNum || 0;
            ...
            render: function (element) {
                ...
                if (settings.extrabar) {
                    content.append('<div class="extra-bar" />');
                }
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (settings.pageChange || (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage))) {
                ...
                switch (settings.scale) {
                    ...
                    case "hours":
                       ...
                       yearArr.push(
                        ...
                        // Added "marign-left:0px" to fix hours-row wrong displacement (1 hour to the left)
                        ...
                        dataPanel.append($('<div class="row" style="margin-left:0px" />').html(horArr.join("")));

                        break;

                    // **Minutes**
                    case "minutes":
                        range = tools.parseMinutesRange(element.dateStart, element.dateEnd, element.scaleStep);

                        var year = range[0].getFullYear();
                        var month = range[0].getMonth();
                        var day = range[0];
                        var hour = range[0].getHours();
                        ...
                        for (var i = 0; i < range.length; i++) {
                            ...
                            // Fill hours
                            var rhour = rday.getHours();
                            if (rhour !== hour) {
                                horArr.push('<div class="row day ' + day_class + '" '
                                        + ' style="width: ' + tools.getCellSize() * minutesInHour + 'px;"> '
                                        + hour
                                        + '</div>');
                                hour = rhour;
                                minutesInHour = 0;
                        ...
                        horArr.push('<div class="row day ' + day_class + '" '
                                 + ' style="width: ' + tools.getCellSize() * minutesInHour + 'px;"> '
                                 + '<div class="fn-label">' + rday.getHours() + '</div></div>');
                        var dataPanel = core.dataPanel(element, range.length * tools.getCellSize());
                        ...
                        dataPanel.append($('<div class="row" style="margin-left:0px" />').html(dowArr.join("")));
                        dataPanel.append($('<div class="row" style="margin-left:0px" />').html(horArr.join("")));
                        dataPanel.append($('<div class="row" style="margin-left:0px" />').html(minArr.join("")));
            ...
            navigation: function (element) {
                ...
                /* トラッカ件数表示 */
                var total = element.rowsNum;
                var r1 = element.pageNum * settings.itemsPerPage + 1;
                var r2 = r1 + settings.itemsPerPage - 1;
                if (total < r2) {
                    r2 = total;
                }
                var pstat = settings.pagestat;
                pstat = pstat.replace(/{from}/, r1).replace(/{to}/, r2).replace(/{total}/, total);
                if (element.rowsNum == 0) pstat = "";
                        ...
                        .append($('<div class="nav-slider" />')
                            .append($('<div class="nav-slider-left" />')
                                ...
                                .append($('<div class="page-number"/>')
                                        .append($('<span/>')
                                            .html(element.pageNum + 1 + ' of ' + (element.pageCount || 1))))
                                ...
                                .append($('<span role="button" class="nav-link nav-prev-week"/>')
                                    ...
                                    .click(function () {
                                        ...
                                        } else if (settings.scale === 'minutes') {
                            ...
                            .append($('<div class="nav-slider-right" />')
                                ...
                                .append($('<span class="page-stat">' + pstat + '</span>'))
                            )
                        );
            ...
            // the entire chart
            createProgressBar: function (days, cls, desc, label, dataObj, option) {
                ...
                if (option) {
                    if (option.style) {
                        bar.css(option.style);
                    }
                    if (option.subWidth) {
                        bar.width(bar.width() - option.subWidth);
                    }
                }
                ...
                $.each(element.data, function (i, entry) {
                    if (settings.pageChange || (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage))) {
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            if (minDate && dtTo < minDate) return;
                            if (maxDate && maxDate < dtFrom) return;
                            if (minDate && dtFrom < minDate) dtFrom = new Date(minDate.getTime());
                            if (maxDate && maxDate < dtTo) dtTo = new Date(maxDate.getTime());

                            switch (settings.scale) {
                                // **Hourly data**
                                case "hours":
                                    var dFrom = tools.genId(dtFrom.getTime(), element.scaleStep);
                                    var from = $(element).find('#dh-' + dFrom);

                                    var dTo = tools.genId(dtTo.getTime(), element.scaleStep);
                                ...
                                case "minutes":
                                    ...
                                    var top = tools.getCellSize() * 6 + 2 + parseInt(topEl.attr("offset"), 10);
                                    _bar.css({ 'margin-top': top, 'margin-left': Math.floor(cFrom) });
                ...
                if ((element.pageNum + val) >= 0 && (element.pageNum + val) < Math.ceil(element.rowsNum / settings.itemsPerPage)) {
                    core.waitToggle(element, true, function () {
                        ...
                        if (settings.pageChange) {
                            settings.pageChange(element.pageNum);
                        } else {
                            core.init(element);
                        }
                    });
            ...
            zoomInOut: function (element, val) {
                core.waitToggle(element, true, function () {
                    ...
                    if (settings.scale === "minutes") {
                        var scaleSt = element.scaleStep + val * 15;
                        scaleSt = scaleSt <= 1 ? 1 : scaleSt === 16 ? 15 : scaleSt;
                    } else {
                        var scaleSt = element.scaleStep + val * 6;
                        scaleSt = scaleSt === 0 ? 1 : scaleSt === 7 ? 6 : scaleSt;
                    }
                    ...
                    if (settings.scale === "hours" && scaleSt >= 13) {
                        ...
                        headerRows = 4;
                        scaleSt = 13;
                    } else if (settings.scale === "hours" && scaleSt < 1) {
                        scale = "minutes";
                        headerRows = 6;
                        scaleSt = 30;
                    } else if (settings.scale === "minutes" && scaleSt >= 31) {
                        scale = "hours";
                        headerRows = 5;
                        scaleSt = 1;
        ...
        var tools = {
            ...
            getMaxDate: function (element) {
                ...
                if (settings.maxDate) maxDate = new Date(settings.maxDate.getTime());

                switch (settings.scale) {
                    case "minutes":
                        maxDate.setMinutes(Math.floor(maxDate.getMinutes() / element.scaleStep) * element.scaleStep);
                        if (!settings.maxDate) {
                            maxDate.setMinutes(maxDate.getMinutes() + element.scaleStep * 3);
                        }
                        break;
                    case "hours":
                        maxDate.setHours(Math.floor((maxDate.getHours()) / element.scaleStep) * element.scaleStep);
                        if (!settings.maxDate) {
            ...
            getMinDate: function (element) {
                ...
                if (settings.minDate) minDate = new Date(settings.minDate.getTime());

                switch (settings.scale) {
                    case "minutes":
                        minDate.setMinutes(Math.floor(minDate.getMinutes() / element.scaleStep) * element.scaleStep);
                        if (!settings.minDate) {
                            minDate.setMinutes(minDate.getMinutes() - element.scaleStep * 3);
                        }
                        break;
                    case "hours":
            ...
            parseMinutesRange: function (from, to, scaleStep) {
                var current = new Date(from);
                var end = new Date(to);
                var ret = [];
                var i = 0;
                ...
                do {
                    ret[i] = new Date(current.getTime());
                    current.setMinutes(current.getMinutes() + scaleStep);
                    current.setMinutes(Math.floor((current.getMinutes()) / scaleStep) * scaleStep);

                    if (current.getHours() !== ret[i].getHours()) {
            ...
            genId: function (ticks) {
                ...
                switch (settings.scale) {
                    ...
                    case "minutes":
                        ...
                        if (arguments.length >= 2) {
                            minute = Math.floor(t.getMinutes() / arguments[1]) * arguments[1];
                        }
                        return (new Date(t.getFullYear(), t.getMonth(), t.getDate(), t.getHours(), minute)).getTime();
                    case "weeks":
                        var y = t.getFullYear();
        ...
        this.each(function () {
            ...
            */
            ...
            switch (settings.scale) {
                case "minutes": this.headerRows = 6; this.scaleStep = 1; break;
